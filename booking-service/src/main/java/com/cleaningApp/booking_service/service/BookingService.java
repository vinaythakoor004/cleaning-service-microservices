...
import com.cleaningApp.booking_service.entity.BookingResponse;
...
public class BookingService {
    ...
    public Booking saveBooking(Booking booking) {
        ...
        this.sendEmailAndNotification(booking, "bookingEvents-out-0", "New Booking", false);        
        return saved;
    }
    ...
    public Booking updateBooking(Long id, Booking updatedBooking) {
        return bookingRepository.findById(id).map(existingBooking -> {
            ...
            this.sendEmailAndNotification(updatedBooking, "bookingEvents-out-0", "Updated Booking", true);
            return saved;
...
//        streamBridge.send("bookingEvents-out-0", "Booking with ID " + id + " deleted.");
    ...
    private void sendEmailAndNotification(Booking booking, String notificationEvent, String msg, Boolean isUpdate) {
    	BookingResponse bookingResponse = new BookingResponse(booking, msg);
        // Publish a "booking created" event to Kafka
        streamBridge.send(notificationEvent, bookingResponse); // "bookingEvents-out-0" is the binding name from application.yml
